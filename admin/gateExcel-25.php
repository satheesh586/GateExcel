...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" id="emailsform">
    E-mails(camma separated or each eamil in a line):</br><textarea name="email" rows="5" cols="40"><?php /*echo $comment;*/?></textarea><br>
    <input type="radio" name="exam" <?php if (isset($exam) && $exam=="GateExcel135_Server1") echo "checked";?> value="GateExcel135_Server1">GatExcel135<br>
    <input type="radio" name="exam" <?php if (isset($exam) && $exam=="GateExcel25") echo "checked";?> value="GateExcel25">GatExcel25<br>
    <br><br>
...
function sendmailer($to,$username,$pass,$exam){
    ...
    $message = "<h2>Welcome to ".$exam." </h2>";
    $message .="<p>Hi,</br> Please find your credentials for the test series ".$exam." below</p>";
    $message .="<ul>
    ...
    $instanceName=$exam." starts today :)";

    $body = Swift_Message::newInstance($instanceName)
...
foreach ($email as $value){
    ...
    if ($exam=="GateExcel135_Server1") {
        $exam_print="GateExcel-135";
    }elseif ($exam=="GateExcel25") {
        $exam_print="GateExcel-25";
    }
    ...
    echo "<h3 style=\"color:#122afc\">" . $user_name . K_NEWLINE . " : ".$exam_print."</h3>";
    ...
    $sql="SELECT group_id FROM ".K_TABLE_USER_GROUP." WHERE group_name='".$exam."'";
    ...
    if($r){
        ...
        sendmailer(stripslashes(rtrim($user_email)),stripslashes(rtrim($user_name)),stripslashes(rtrim($newpassword)),stripslashes(rtrim($exam)));
    }
}
