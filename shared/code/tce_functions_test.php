                ...
                if (!$expired) {
                    switch ($test_status) {
                        ...
                        case 3: { // 3 = all questions have been answered
                                // continue test
                                $str .= '<a href="tce_test_execute.php?testid=' . $m['test_id'] . '" title="' . $l['h_continue'] . '" class="ctn-btn">' . $l['w_continue'] . '</a>';
                                break;
                            }
...
function F_isValidTestUser($test_id, $user_ip, $test_ip) {
    ...
    // check user's group
    if (F_count_rows(K_TABLE_USERGROUP . ', ' . K_TABLE_TEST_GROUPS, 'WHERE usrgrp_group_id=tstgrp_group_id
...
function F_printTestInfo($test_id, $showip = false) {
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            $str .= '<div class="tceformbox-detail">';
            $str .= '<div class="instruction-header">You are about to begin the test</div>';
            $str .='<div class="instruction-body">';
            $str .= '<ul class="instruction_details">';
            $str .= '<li>Test Name <b>' . htmlspecialchars($m['test_name'], ENT_NOQUOTES, $l['a_meta_charset']) . '</b></li>';
            $str .= '<li>Start time of test is  <b>' . $m['test_begin_time'] . '</b></li>';
            $str .= '<li>Total duration of test is  <b>' . $m['test_duration_time'] . ' ' . $l['w_minutes'] . '</b></li>';
            /*$str .= '<li> Per question score point  <b>' . $m['test_score_right'] . '</b></li>';
            $str .= '<li> Per <b>wrong</b> answer the  <b>' . $m['test_score_wrong'] . '</b> would be deducted</li>';
            $str .= '<li> Points for no answer <b>' . $m['test_score_unanswered'] . '</b></li>';
            $str .= '<li> Maximum score per answer <b>' . $m['test_max_score'] . '</b></li>';
            $str .= '<li> Mininum score to pass the test <b>' . $m['test_score_threshold'] . '</b></li>';
            $str .= '<li> Result of user  ' . $boolval[intval(F_getBoolean($m['test_results_to_users']))] . '</li>';
            $str .= '<li> Report to user ' . $boolval[intval(F_getBoolean($m['test_report_to_users']))] . '</li>';
            $str .= '<li> Test Repeatable ' . $boolval[intval(F_getBoolean($m['test_repeatable']))] . '</li>';*/
            $str .= '</ul>';
            $str.='</div>';
...
function F_questionForm($test_id, $testlog_id, $formname) {
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            ...
            $str.='<div class="help_data"><span class="useful-data-icon"></span><a href="#" id="Data" title="Data">Useful Data</a></div>';
            $str.='<div class="help_calc"><span class="calulator-icon"></span><a href="#" id="Calculator" title="Calculator">Calculator</a></div>';
            $str.='</div>';
            $str .= '<div class="tcecontentbox question">' . K_NEWLINE;
            //fieldset
            //$str .= '<legend>';
            //$str .= $l['w_question'];
                    ...
                    while ($ma = F_db_fetch_array($ra)) {
                        ...
                        switch ($m['question_type']) {
                            case 1: {
                                    ...
                                    $str .= '<input type="radio" class="radio-custom" name="answpos" id="answpos_' . $anspos . '" value="' . $anspos . '"';
                                    ...
                                    $str .= ' />&nbsp;';
                                    $str .= '<label class="" for="answpos_' . $anspos . '">';
...
//			$str .= '</acronym>';
                        ++$i;
                       $class=(!empty($m['testlog_change_time'])) ? ("class='answered'"): (!empty($m['testlog_display_time']) ? "class='displayed'" : "");
                    if ($m['testlog_id'] != $testlog_id) { 
                        $str .= '<li>';
                        $str .= '<input type="submit" name="jumpquestion_' . $m['testlog_id'] . '" id="jumpquestion_' . $m['testlog_id'] . '" value="' . $i . '" title="' . F_tcecodeToTitle($m['question_description']) . '" '.$class.'/> ';
                        if ($testlog_id_last == $testlog_id) {
                            $testlog_id_next = $m['testlog_id'];
                        }
                    } else {
                        $str .= '<li>';
                        $str .= '<input type="button" name="jumpquestion_' . $m['testlog_id'] . '" id="jumpquestion_' . $m['testlog_id'] . '" value="' . $i . '" title="' . F_tcecodeToTitle($m['question_description']) . '" disabled="disabled"  class="selected"/> ';
...
//                        $str .= '&nbsp;';
    ...
    if (!$question_timer) {
        $navlink .= '<input type="submit" name="prevquestion" id="prevquestion" class="prev-btn" title="' . $l['w_previous'] . '" value="&lt; ' . $l['w_previous'] . $qprev . '"';
        if (($testlog_id_prev <= 0) OR ( $testlog_id_prev > $testlog_id)) {
            $navlink .= ' disabled="disabled"';
        }
        $navlink .= ' />';
        // button for confirm current question
        $navlink .= '<input type="submit" name="confirmanswer" id="confirmanswer" class="conf-btn" value="(' . $qsel . ') ' . $l['w_confirm'] . '" />';
    }
    ...
    if (F_getBoolean($testdata['test_menu_enabled']) AND ( !$disable)) {
        ...
        $rstr .= '<div class="tcecontentbox quesions-list">' . K_NEWLINE; //fieldset
        //$rstr .= '<legend>';
        $rstr .= $l['w_questions'];
        //$rstr .= '</legend>'.K_NEWLINE;
        $rstr .= '<ul class="qlist">' . K_NEWLINE . $str . '</ul>' . K_NEWLINE;
...
function F_testComment($test_id) {
    ...
    if (F_getBoolean($td['test_comment_enabled'])) {
        ...
        $str .= '<label for="testcomment" class="testcomment-lable">' . $l['w_comment'] . '</label><br />';
